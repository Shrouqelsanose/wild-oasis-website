...
import { useReservation } from "./RerevationContext";
...
function DateSelector({ settings, cabin, bookedDates }) {
  ...
  return (
    <div className="flex flex-col pl-10 py-10 justify-between">
      <DayPicker
      ...
      <div className="flex  items-center justify-between px-8 bg-[var(--color-accent-500)] text-[var(--color-primary-800)] h-[72px]">
        ...
        {range.from || range.to ? (
          <button
            className="border border-[var(--color-primary-800)] py-2 px-4 text-sm font-semibold"
            onClick={() => resetRange()}>
            Clear
          </button>
        ) : null}
